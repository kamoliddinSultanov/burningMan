...
// Instantiate the EventFunctions class
$eventFunctions = new EventFunctions();

// Verify session
$eventFunctions->verifyUserSession();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['join_event']) && $event_id) {
        $result = $eventFunctions->joinEvent($user_id, $event_id);
    } elseif (isset($_POST['leave_event']) && $event_id) {
        $result = $eventFunctions->leaveEvent($user_id, $event_id);
    }
}

// Fetch events data
$user_events = $eventFunctions->getUserEvents($user_id);
$all_events = $eventFunctions->getAvailableEvents($user_id);
?>
